...
-- Temporarily disable foreign key constraint for equipment_listings
ALTER TABLE equipment_listings DROP CONSTRAINT IF EXISTS equipment_listings_seller_id_fkey;

-- Insert sample equipment listings (without foreign key constraint)
...
-- Re-add the foreign key constraint (optional - you can leave this commented out for now)
-- ALTER TABLE equipment_listings ADD CONSTRAINT equipment_listings_seller_id_fkey FOREIGN KEY (seller_id) REFERENCES profiles(id);
